...
export default function SelectCoursePage() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        setIsLoading(true)
        const response = await fetch('http://localhost:8003/categories/public/details?active_only=true')
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:8003/courses/public/by-category/${formData.category_id}?active_only=true`)
