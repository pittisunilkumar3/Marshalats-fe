...
export default function CourseAssignmentPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        const [studentsRes, coursesRes, branchesRes] = await Promise.all([
          fetch('http://localhost:8001/users/students/details', {
            headers: { 'Authorization': `Bearer ${token}` }
          }),
          fetch('http://localhost:8001/courses/public/all'),
          fetch('http://localhost:8001/branches', {
            headers: { 'Authorization': `Bearer ${token}` }
          })
        ])
  ...
  useEffect(() => {
    const fetchDurations = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:8001/durations/public/by-course/${formData.course_id}`)
        if (response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:8001/enrollments', {
        method: 'POST',
        headers: {
